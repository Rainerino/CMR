...
% see the LICENSE file included with this software
    ...
    %% METHODS
        ...
        %   See also controller/execute
            ...
            fprintf('%0.3f  %0.3f \n' ,obj.robot.wheel_radius, obj.robot.wheel_base_length);
...
%                 fprintf('already in state %s\n', name);
        ...
        %   measured wheel encoder tick counts are stored in prev_ticks.
            ...
            %% START CODE BLOCK %%
            dt = 0.033;
            vl =  m_per_tick * (left_ticks - prev_left_ticks) / dt;
            vr =  m_per_tick * (right_ticks - prev_right_ticks) / dt;  
            x_dt = R/L * (vr + vl) * cos(theta);
            y_dt = R/L * (vr + vl) * sin(theta);
            theta_dt = R/L * (vr - vl);
            %% END CODE BLOCK %%
